...
File name: csma.cpp
Compilation command : g++ csma.cpp -o csma
*/
#include <iostream>
#include <cmath>
#include <vector>
...
#include <ctime>
using namespace std;
double T = 100.0;       //simulation time
...
struct Node //structure of every node
{
    ...
    double T_arr;     // time when first bit reaches node
    double T_waiting; //waiting time
};
...
{
    int transmitter = 0;        //index of the transmitting node
    for (int j = 0; j < N; j++) //Initializing all variables
    {
    ...
    for (int k = 0; k < N; k++)
    {
        while (!n[k].q.empty())
        {
            ...
            //cout << "Transmission starts with node " << transmitter << endl;
            for (int i = 0; i < N; i++)
            {
                if (i != transmitter)
                {
            ...
            if (flag == 0) //Successful Transmission
            {
                ...
                for (int i = 0; i < N; i++) //Readjusting the arrival times if the channel is busy
                {
                    if (!n[i].q.empty())
                    {
                        for (itr2 = n[i].q.begin(); itr2 != n[i].q.end(); itr2++)
                        {
                            if ((*itr2).time > n[i].T_arr && (*itr2).time < (n[i].T_arr + (L / R)))
                            {
                                (*itr2).time = n[i].T_arr + (L / R);
                            }
                        }
                    }
                }
    ...
    cout<<"Throughput in mbps for n= "<<N<<": "<<s2*L/(double)(T*1000000)<<endl;//Printing the throughput
}
int main()
{
